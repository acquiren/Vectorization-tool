...
# -*- coding: utf-8 -*-
"""
SuperSVG coarse阶段训练主脚本

使用方法：
1. 在 IDE 中打开此脚本
2. 准备数据集，修改 data_path 参数
3. 运行脚本进行训练
"""
...
class AttrImgDataset(Dataset):
    ...
    def __init__(self, data_root, bs=24):
        ...
        self.l = len(self.paths)  # 图像总数
        
        # 定义图像变换
        self.transform = transforms.Compose([
            transforms.Resize((width, width)),  # 调整大小
            transforms.ToTensor(),  # 转换为张量
            ])
        # 定义128x128的大小调整
        self.resize_128 = transforms.Resize([128, 128])
        # 定义width x width的大小调整
    ...
    def __getitem__(self, index):
        ...
        Args:
            index: 样本索引（实际随机选择一个样本）
        
        Returns:
            tuple: (image, mask) 图像和对应的掩码
        """
        # 随机选择一个图像索引
...
def main(args):
    ...
    # 按照timm的方式设置参数组（bias和norm层的weight decay设为0）
